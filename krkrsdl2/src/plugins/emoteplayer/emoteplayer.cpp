...
// krkrsdl2common is a static archive.  The registration records below are
// created only by static initializers, so the linker otherwise has no symbol
// reason to extract this translation unit from the archive.  PluginImpl calls
// this no-op anchor before ncbAutoRegister::LoadModule().
