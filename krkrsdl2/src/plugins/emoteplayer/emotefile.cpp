...
emotemetadata::emotemetadata(emotefile* filePtr, uint32_t startOffset) : _filePtr(filePtr)
{
    ...
    for (auto itm : _tmpList)
    {
        ...
        it = _base.find("enabled");
        if (it != _base.end())
        {
            filePtr->parseNumber(tmpNum, it->second);
            tmp->enabled = tmpNum != 0;
        }
        it = _base.find("blinkEnabled");
        if (it != _base.end())
        {
            filePtr->parseNumber(tmpNum, it->second);
            tmp->blinkEnabled = tmpNum != 0;
        }
        ...
        if (tmp->blinkIntervalMax < tmp->blinkIntervalMin)
            std::swap(tmp->blinkIntervalMin, tmp->blinkIntervalMax);
        tmp->blinkIntervalMin = std::max(0, tmp->blinkIntervalMin);
        tmp->blinkIntervalMax = std::max(tmp->blinkIntervalMin, tmp->blinkIntervalMax);
        tmp->uid = std::uniform_int_distribution<int32_t>(tmp->blinkIntervalMin,
                                                           tmp->blinkIntervalMax);
        uint32_t seed = 2166136261u;
        for (unsigned char c : tmp->label)
            seed = (seed ^ c) * 16777619u;
        tmp->randomEngine.seed(seed ? seed : 1u);
        KRKRNS_LOG("[emote] eye control label=%s enabled=%d autoBlink=%d frames=%d interval=%d..%d",
                   tmp->label.c_str(), tmp->enabled ? 1 : 0,
                   tmp->blinkEnabled ? 1 : 0, tmp->blinkFrameCount,
...
void emoteicon::ensureLoad()
{
    if (data == nullptr)
    {
        ...
        KRKRNS_LOG("[emote] icon decoded %p bytes=%d", this, datasize);
...
bool emotefile::load(const ttstr& filePath)
{
    ...
    if (std::memcmp(sign, lzfs, 4) == 0) // lzfs
    {
    ...
    switch (auto type = static_cast<PSB::PSBObjType>(typeByte))
    {
        ...
        case PSB::PSBObjType::StringN4:
        {
            ...
            while (true)
            {
                ...
                {
                    tjs_string utf16;
                    if (!TVPUtf8ToUtf16(utf16, str))
                        return tTJSVariant();
                    return tTJSVariant(ttstr(utf16));
                }
                str.append(1, val8);
            }
        ...
        case PSB::PSBObjType::Objects:
        {
            ...
            {
                tjs_string keyName16;
                if (!TVPUtf8ToUtf16(keyName16, namesCache.at(_objsNamesIdx.at(i))))
                    continue;
                ttstr keyName(keyName16);
