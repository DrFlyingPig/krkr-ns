...
bool emotenoderef::draw(krkrsdl3::iTVPRenderBackend* renderer, void* target, emotelimit lim, void* maskTarget, bool inMaskPass)
{
    ...
    if (renderMethod.at(0).hasStencil && maskTarget != 0)
    {
        ...
        static int maskDumps = 0;
        if (maskDumps < 3)
        {
            maskDumps++;
            int pitch = 0;
            uint8_t* pixels = renderer->LockTarget(maskTarget, pitch);
            if (pixels)
            {
                SDL_Surface* shot = SDL_CreateRGBSurfaceWithFormatFrom(
                    pixels, 1280, 720, 32, pitch, SDL_PIXELFORMAT_ABGR8888);
                if (shot)
                {
                    char path[128];
            ...
            renderer->UnlockTarget(maskTarget);
        }
#endif
    }
    ...
    {
        surfaceLogs++;
        const int n = (int)renderMethod.size();
        for (int i = 0; i < n; i++)
        {
            if (i >= 3 && i < n - 3 && n > 6) continue;
            const auto& rm = renderMethod.at((size_t)i);
            KRKRNS_LOG(
                "[emote] surf[%d/%d] type=%d w=%.1f h=%.1f org=(%.1f,%.1f) coord=(%.1f,%.1f) zx=%.3f zy=%.3f opa=%.2f",
                i, n, rm.type, rm.width, rm.height, rm.originX,
                rm.originY, rm.currCoordx, rm.currCoordy, rm.currZx, rm.currZy, rm.opa);
        }
...
void emoteengine::updateEyeControl(float tick, bool isMain)
{
    ...
    {
        if (!itm || !itm->enabled || !itm->blinkEnabled || itm->blinkFrameCount <= 0)
            continue;
        // 初始化tick
        if (!itm->hasStart)
        {
        ...
        {
            // Keep RNG state between blinks.  Reconstructing an engine here
            // selected the same interval every time and made eye motion look
            // mechanical even in assets that enable automatic blinking.
            itm->currWaitInterval = itm->uid(itm->randomEngine);
        }
        ...
        else // 未播放时进行等待
        {
            if (tick - itm->lastTick > itm->currWaitInterval)
            {
                itm->isBlinking = true;
                itm->lastTick = tick;
                TVPConsoleLog("auto blink start label=%s tick=%.2f wait=%d",
                              itm->label.c_str(), tick, itm->currWaitInterval);
                // 获取初始值
                auto varPos = _mainfile->_metadata->_varList.find(itm->label);
                if (varPos != _mainfile->_metadata->_varList.end())
                {
