// savestruct.dll: Dictionary/Array "struct string" serializer.
//
// Ported from Kirikiroid2 (src/plugins/saveStruct.cpp).  It (re)adds the
// textual save-data format KAG titles use: Array.save2 / saveStruct2 /
// toStructString, and the same three on Dictionary.  The Dictionary members
// are attached as class members (TJS_STATICMEMBER), exactly like the desktop
// plugin, so titles reach them through
// "(Dictionary.saveStruct2 incontextof dict)(filename)".
//
// The upstream file includes "PluginIntf.h"; here the engine side entry points
// (TVPCreateStream, tTVPMemoryStream, TVPExecuteExpression,
// TVPThrowExceptionMessage) come from tp_stub.h/StorageIntf.h/UtilStreams.h.
#include "tp_stub.h"
...
void tTVPStringStream::write(tTVInteger var) {
	tTJSVariant v(var);
	tTJSVariantString *vs = v.AsString();
	stream->Write(vs->operator const tjs_char *(), vs->GetLength() * sizeof(tjs_char));
	vs->Release();
}
...
public:
	...
	static tjs_error TJS_INTF_METHOD save2(tTJSVariant *result,
        ...
        tTJSBinaryStream *stream = TVPCreateStream(ttstr(*param[0]), TJS_BS_WRITE);
...
//                 writer.hex = true;
	...
	static tjs_error TJS_INTF_METHOD toStructString(tTJSVariant *result,
		...
		if (result) {
			...
			getArrayString(objthis, &writer);
			// The upstream array path relied on the fresh stream buffer being
			// zero filled; the Dictionary path below already terminates the
			// string explicitly, so do the same here.
