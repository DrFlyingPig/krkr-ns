...
// krkrsdl2common is a static archive: the registration records below exist only
// as static initializers, so the linker would otherwise drop this translation
// unit entirely.  PluginImpl calls this anchor before ncbAutoRegister::LoadModule.
...
static bool GetLayerSize(iTJSDispatch2* lay, long& w, long& h, long& pitch)
{
    ...
    // レイヤイメージは在るか？
    tTJSVariant val;
    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("hasImage"), 0, &val, lay)) ||
        (val.AsInteger() == 0))
        return false;

    // レイヤサイズを取得
    val.Clear();
    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("imageWidth"), 0, &val, lay)))
        return false;
    w = (long)val.AsInteger();

    val.Clear();
    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("imageHeight"), 0, &val, lay)))
        return false;
    h = (long)val.AsInteger();

    // ピッチ取得
    val.Clear();
    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferPitch"), 0, &val, lay)))
        return false;
...
static bool GetLayerBufferAndSize(iTJSDispatch2* lay, long& w, long& h, WrtRefT& ptr, long& pitch)
{
    ...
    // バッファ取得
    tTJSVariant val;
    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferForWrite"), 0, &val, lay)))
        return false;
                               ...
                               iTJSDispatch2* dst)
{
    ...
    // バッファ取得
    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBuffer"), 0, &val, src)))
        return false;
    sbuf = reinterpret_cast<ReadRefT>(val.AsInteger());

    if (TJS_FAILED(layerClass->PropGet(0, TJS_W("mainImageBufferForWrite"), 0, &val, dst)))
        return false;
