// wutcwf.dll: TCWF (TwinVoice) wave decoder for WaveSoundBuffer.
//
// Ported from Kirikiroid2 (src/plugins/wutcwf.cpp), which is a verbatim port
// of the original Windows plugin (MS-ADPCM + IMA-ADPCM blocks with the
// TwinVoice "unexpected peak" revision).  Registering a tTVPWaveDecoderCreator
// is all it needs; the engine's decoder manager tries it for every audio
// storage and the TCWF0 magic decides.
