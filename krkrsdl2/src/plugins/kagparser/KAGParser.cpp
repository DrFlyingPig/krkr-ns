				...
				if(condition && ExcludeLevel == -1)
				{
					if(tagkind == tag_emb || (ismacro && tagkind==tag_other))
					{
						...
						if(!ismacro)
						{
							...
							// KAGParserEx compatibility: the historical parser escaped
							// embedded '[' characters unconditionally.  KAGParserEx adds
							// escape=false so generated tag strings can be parsed as tags.
							// Defaulting to true preserves the original KAGParser behavior.
							bool escape = true;
							DicObj->PropGet(0, __escape_name.c_str(),
								__escape_name.GetHint(), &val, DicObj);
