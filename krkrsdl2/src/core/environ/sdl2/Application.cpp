...
#ifdef __SWITCH__
// KRKR-ns patch: declared in SDLApplication.cpp
extern void switch_game_mount();
#endif
#include "EventIntf.h"
...
#endif
tjs_string ExePath() {
	// KRKR-ns patch: no static caching — __SWITCH__ value depends on game
	// mode (romfs vs sdmc game dir) and would pin the pre-mount value.
	tjs_string exepath;
#if defined(_WIN32)
	{
	...
	// KRKR-ns patch: homebrew NROs have no OS-provided executable path;
	// report the game location (file media, storage-legal name). In game
	// mode this is the sdmc game dir so System.exepath etc. resolve plugin/
	// and data paths against the actual game data; otherwise the romfs
	...
	if (exepath.empty())
	{
		...
		{
			exepath = tjs_string(TJS_W("file://"));
			exepath += krkrsdl2_game_dir.AsStdString();
			exepath += TJS_W("krkrsdl2.nro");
		}
		else
			exepath = tjs_string(TJS_W("file://romfs:/krkrsdl2.nro"));
	}
#endif
...
bool tTVPApplication::StartApplication( int argc, tjs_char* argv[] ) {
	...
	KRKRNS_LOG("[app] StartApplication entry");
	CheckConsole();
	KRKRNS_LOG("[app] CheckConsole ok");
	...
	try {
		...
		KRKRNS_LOG("[app] TVPInitScriptEngine entry");
	TVPInitScriptEngine();
	KRKRNS_LOG("[app] TVPInitScriptEngine ok");
		engine_init = true;
		...
		KRKRNS_LOG("[app] TVPInitializeBaseSystems entry");
		TVPInitializeBaseSystems();
		KRKRNS_LOG("[app] TVPInitializeBaseSystems ok");
		...
		KRKRNS_LOG("[app] before TVPExecuteUserConfig");
		if(TVPExecuteUserConfig()) return true;
		KRKRNS_LOG("[app] TVPExecuteUserConfig ok");

#ifdef KRKRSDL2_ENABLE_ASYNC_IMAGE_LOAD
		image_load_thread_ = new tTVPAsyncImageLoader();
#endif

		KRKRNS_LOG("[app] TVPSystemInit entry");
		TVPSystemInit();
		KRKRNS_LOG("[app] TVPSystemInit ok");
		...
		KRKRNS_LOG("[app] before TVPInitializeStartupScript (dir=%d)", (int)TVPProjectDirSelected);
#ifdef __SWITCH__
		// KRKR-ns patch: mount sdmc game data (xp3 archives) and, if a game
		// is present, point the data path at it BEFORE the startup script is
		// resolved — the engine then loads the game's own startup.tjs.
		switch_game_mount();
#endif
		if(TVPProjectDirSelected) TVPInitializeStartupScript();
		KRKRNS_LOG("[app] TVPInitializeStartupScript ok");
		...
		TVPOnError();
		KRKRNS_LOG("[app] startup exception: %s", exception.what());
		if(!TVPSystemUninitCalled)
			ShowException(exception.what());
	} catch( const TJS::eTJSScriptError &e ) {
		TVPOnError();
		KRKRNS_LOG("[app] startup script exception: %s", e.GetMessage().c_str());
		if(!TVPSystemUninitCalled)
			ShowException( e.GetMessage().c_str() );
	} catch( const TJS::eTJS &e) {
		TVPOnError();
		KRKRNS_LOG("[app] startup TJS exception: %s", e.GetMessage().c_str());
		if(!TVPSystemUninitCalled)
			ShowException( e.GetMessage().c_str() );
...
void tTVPApplication::Run() {
...
#ifdef __EMSCRIPTEN__
		emscripten_run_script("FS.syncfs(false, function (err) { Module._" "syncfs_is_finished" "(); });");
#endif
	}
	if (done)
	{
	...
	{
		const Uint64 prof_t4 = SDL_GetPerformanceCounter();
		const double prof_freq = (double)SDL_GetPerformanceFrequency();
		krkrsdl2_prof_seg(KRKRNS_PROF_SEG_EVENTS,
			(double)(prof_t1 - prof_t0) * 1000.0 / prof_freq);
		krkrsdl2_prof_seg(KRKRNS_PROF_SEG_DISPATCH,
			(double)(prof_t2 - prof_t1) * 1000.0 / prof_freq);
		krkrsdl2_prof_seg(KRKRNS_PROF_SEG_TICKBEAT,
			(double)(prof_t3 - prof_t2) * 1000.0 / prof_freq);
		krkrsdl2_prof_seg(KRKRNS_PROF_SEG_WAIT,
			(double)(prof_t4 - prof_t3) * 1000.0 / prof_freq);
		krkrsdl2_prof_accum_frame();
	}
}
