...
public:
	...
	static tjs_string GetDataPathDirectory( tjs_string datapath, const tjs_string& exename ) {
		...
		// KRKR-ns patch: switch SDL2 (built --disable-filesystem) has no
		// usable GetPrefPath. The data path doubles as the game data root:
		// the packaged game lives in the RomFS. (Save dirs get overlaid on
		// sdmc in the save-layer patches, Phase 4.)
		return TJS_W("file://romfs:/");
#else
