...
static void TVPShowSimpleMessageBox(const ttstr & text, const ttstr & caption)
{
	...
	std::string c_utf8;
	const bool text_converted = TVPUtf16ToUtf8(t_utf8, t_utf16);
	const bool caption_converted = TVPUtf16ToUtf8(c_utf8, c_utf16);
	KRKRNS_LOG("[msgbox] TVPShowSimpleMessageBox entry: [%s] [%s]", text_converted ? t_utf8.c_str() : "<utf16 conversion failed>", caption_converted ? c_utf8.c_str() : "<utf16 conversion failed>");
#ifdef __SWITCH__
	// KRKR-ns diagnostic: log the TJS call stack so a platform-conditional
	// dialog can be traced back to the exact script and line.
	{
		ttstr trace = TJSGetStackTraceString(12, TJS_W(" <- "));
		std::string utf8;
		for (tjs_uint i = 0; i < trace.GetLen(); ++i)
		{
		...
		KRKRNS_LOG("[msgbox] TJS stack: %s", utf8.c_str());
	}
#endif
#ifdef __SWITCH__
	// KRKR-ns patch: never show the SDL message box on Switch — the real
	// device crashed the whole system when the exception dialog path ran
	// (observed with a script exception). Log instead.
	(void)t_utf8; (void)c_utf8; (void)t_utf16; (void)c_utf16;
#else
	if (text_converted && caption_converted)
	{
		SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, c_utf8.c_str(), t_utf8.c_str(), nullptr);
	}
#endif
	KRKRNS_LOG("[msgbox] TVPShowSimpleMessageBox exit");
}
//---------------------------------------------------------------------------
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/getArgument)
{
...
#ifdef __SWITCH__
	{
		// KRKR-ns diagnostic: print the full argument name in UTF-8
		// (AsStdString truncates at the first 0x00 on Switch).
		std::string utf8;
		for (tjs_uint i = 0; i < name.GetLen() && i < 150; ++i)
		{
		...
		KRKRNS_LOG("[getArgument] %s", utf8.c_str());
	}
#endif
	...
	if (result && result->Type() != tvtVoid)
	{
		...
		KRKRNS_LOG("[getArgument] -> %s (res=%d)", utf8r.c_str(), (int)res);
	}
	else
		KRKRNS_LOG("[getArgument] -> <void> (res=%d)", (int)res);
#endif
