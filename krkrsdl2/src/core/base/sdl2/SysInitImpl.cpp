...
#ifdef __SWITCH__
// TVP_CPU_HAS_* flags for the simde-kernel activation below (Phase 1).
...
void TVPBeforeSystemInit()
{
	...
	// memory usage
	{
		...
		// SDL's Switch backend currently reports 0 MB.  Kirikiri interprets
		// that as an extremely low-memory machine: it disables the script
		// segment cache, frees reusable layer buffers, and sets the graphics
		// cache to zero.  Options screens then reopen/decode the same PSB
		// pieces on every state change.  Application/title-override mode can
		// safely retain the 40 MiB cache.  Album is a constrained LibraryApplet,
		// so keep only 8 MiB for the launcher and refuse large game startup.
		TVPTotalPhysMemory = static_cast<tjs_uint64>(SDL_GetSystemRAM()) * 1024 * 1024;
		if (TVPTotalPhysMemory == 0)
		{
			...
			TVPTotalPhysMemory = (application ? 512ULL : 128ULL) * 1024 * 1024;
		}
#else
...
//	TVPGraphicCacheSystemLimit = 1*1024*1024; // DEBUG
...
#endif
#ifdef __SWITCH__
	// KRKR-ns Phase 1: TVPCPUType is 0 on ARM (the x86 CPUID detector is
	// compiled out), so the simde-backed SSE2 kernels in this build were
	...
	if (tvpglScalar)
	{
		// Explicitly mask the flags off — TVPCPUType may carry stale bits
		// from boot-time init, and TVPGL_SSE2_Init gates on them.
		TVPCPUType &= ~(TVP_CPU_HAS_SSE | TVP_CPU_HAS_SSE2 | TVP_CPU_HAS_SSE3 |
						TVP_CPU_HAS_SSSE3 | TVP_CPU_HAS_SSE41 | TVP_CPU_HAS_SSE42);
	}
	else
	{
		TVPCPUType = TVP_CPU_HAS_SSE | TVP_CPU_HAS_SSE2 | TVP_CPU_HAS_SSE3 |
	...
	KRKRNS_LOG("[ns] tvpgl: simde kernels %s (TVPCPUType=0x%x)",
		tvpglScalar ? "OFF (scalar)" : "ON", (unsigned)TVPCPUType);
#endif
	TVPGL_SSE2_Init();
	...
	if(TVPGetCommandLine(TJS_W("-timerprec"), &opt))
	{
        ...
        // draw thread num
#ifdef __SWITCH__
        // Large Kirikiri layer blends are split by scanlines.  Let the
        // existing adaptive worker pool use the Switch CPU cores; small
        // operations remain single-threaded and avoid scheduling overhead.
        tjs_int drawThreadNum = 0; // auto (SDL_GetCPUCount)
#else
...
#endif
			KRKRNS_LOG("[arg] before GetConfigFileName/options, ExePath=[%s]", ttstr(ExePath()).AsNarrowStdString().c_str());
		options[1] = TVPGetConfigFileOptions(ApplicationSpecialPath::GetConfigFileName(ExePath()));
		KRKRNS_LOG("[arg] config options done");
			...
			KRKRNS_LOG("[arg] datapath=[%s]", TVPNativeDataPath.c_str());
