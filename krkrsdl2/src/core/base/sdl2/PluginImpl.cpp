...
#ifdef __SWITCH__
// Follow Kirikiroid2's InternalPlugins.cpp / PluginImpl.cpp: availability is
...
static bool TVPHasSwitchBuiltin(const ttstr& name)
{
	...
	if (name == TJS_W("kagparser.dll")) class_name = TJS_W("KAGParser");
#endif
#ifdef KRKRSDL2_ENABLE_CSVPARSER
	if (name == TJS_W("csvparser.dll")) class_name = TJS_W("CSVParser");
#endif
#ifdef KRKRSDL2_ENABLE_PSBFILE
	if (name == TJS_W("psbfile.dll")) class_name = TJS_W("PSBFile");
#endif
#ifdef KRKRSDL2_ENABLE_TEXTRENDER
	if (name == TJS_W("textrender.dll")) class_name = TJS_W("TextRenderBase");
#endif
	if (class_name)
	{
		...
		tTJSVariant value;
		const bool present = TJS_SUCCEEDED(global->PropGet(0, class_name, nullptr, &value, global)) &&
			value.Type() == tvtObject && value.AsObjectNoAddRef();
		global->Release();
		return present;
	}
	// Codec plugin names are fulfilled by the core's registered decoders.
...
void TVPLoadPlugin(const ttstr & name)
{
	...
	// Both resolve to the single statically-linked module registered as
	// emoteplayer.dll; without the alias the Motion/EmotePlayer classes
	// never register and E-mote games abort during KAG boot.
	ttstr emote_module = short_name;
	if (emote_module == TJS_W("motionplayer.dll") ||
		emote_module == TJS_W("emotedriver.dll"))
		emote_module = TJS_W("emoteplayer.dll");
	// Force registration records into the static link before LoadModule.
	krkrsdl2_link_emoteplayer_plugin();
	ncbAutoRegister::LoadModule(emote_module);
	if (TVPRegisteredPlugins.find(emote_module) != TVPRegisteredPlugins.end())
	{
...
#endif
	if (TVPHasSwitchBuiltin(short_name))
	{
		if (ns_builtin_plugins.insert(short_name).second)
			TVPAddLog(ttstr(TJS_W("(info) loaded built-in plugin: ")) + short_name);
		return;
	...
	if (ns_unavailable_plugins.insert(short_name).second)
		TVPAddLog(ttstr(TJS_W("(warning) plugin unavailable on Switch: ")) + short_name);
	return;
#endif
#ifdef KRKRSDL2_ENABLE_PLUGINS
	// load plugin
...
bool TVPUnloadPlugin(const ttstr & name)
{
	...
	const ttstr short_name = TVPExtractStorageName(name).AsLowerCase();
	if (ns_builtin_plugins.find(short_name) != ns_builtin_plugins.end())
		return false; // Static classes/media stay alive for the script engine lifetime.
#endif
#ifdef KRKRSDL2_ENABLE_PLUGINS
	tTVPPluginVectorType::iterator i;
	for(i = TVPPluginVector.Vector.begin();
...
#endif
#ifdef __SWITCH__
		tjs_int builtin_index = 0;
		for (const ttstr& name : ns_builtin_plugins)
		{
			tTJSVariant val(name.c_str());
			array->PropSetByNum(TJS_MEMBERENSURE, builtin_index++, &val, array);
		}
#endif
