...
bool TJS_INTF_METHOD tTVPFileMedia::CheckExistentStorage(const ttstr &name)
{
	...
	GetLocalName(_name);
#ifdef __SWITCH__
	// KRKR-ns patch: the emulator's fsdev stat() hangs/fails on sdmc paths —
	// probe with fopen instead (open works; used by KAG plugin/archive
	// existence checks like "file://sdmc:/.../plugin/wargbm.dll").
	{
		std::string fn;
		if (TVPUtf16ToUtf8(fn, _name.AsStdString()))
		{
...
#else
				// KRKR-ns patch: fsdev reports d_type == DT_UNKNOWN for
				// romfs entries; treat unknown as a file (stat() hangs on
				// the current emulator's fsdev, so it cannot be used here).
				bool d_is_file = (entry->d_type == DT_REG || entry->d_type == DT_UNKNOWN);
				if( d_is_file )
#endif
				{
					tjs_char fname[256];
...
//---------------------------------------------------------------------------
static bool ns_gp_is_bare(const tjs_char *p) // not "?/", no ':'
{
	if (p[0] == TJS_W('?') && p[1] == TJS_W('/')) return false;
	for (const tjs_char *q = p; *q; q++) if (*q == TJS_W(':')) return false;
	return true;
}
//---------------------------------------------------------------------------
void TJS_INTF_METHOD tTVPFileMedia::GetLocallyAccessibleName(ttstr &name)
{
	...
	// KRKR-ns patch: "file://?/<raw>" denotes an fsdev device path
	// (romfs:/, sdmc:/, ...); strip the marker as-is. The generic
	// directory-walk below mishandles the ':' inside such paths.
	// In game mode, a bare tail (no ':' device, not "?/") is a relative
	// data-path name: resolve it under the game directory instead of the
	// process cwd (the emulator's fs ignores chdir()).
	extern bool krkrsdl2_game_mode;
	extern ttstr krkrsdl2_game_dir;
	{
		const tjs_char *pp = name.c_str();
		if (!memcmp(pp, TJS_W("file://?/"), 9 * sizeof(tjs_char))) {
			ttstr tail(pp + 9);
			if (krkrsdl2_game_mode && ns_gp_is_bare(tail.c_str()))
			{
				tail = krkrsdl2_game_dir + tail;
			}
			name = tail;
			return;
		}
		// the storage manager strips "file://" before calling the media;
		// accept the bare "?/<device path>" form too
		if (!memcmp(pp, TJS_W("?/"), 2 * sizeof(tjs_char))) {
			ttstr tail(pp + 2);
			if (krkrsdl2_game_mode && ns_gp_is_bare(tail.c_str()))
			{
				tail = krkrsdl2_game_dir + tail;
		...
		// "file://<dev>:/..." (e.g. System.exepath-based KAG plugin paths —
		// "file://sdmc:/...") → strip the scheme, keep the device path.
		if (!memcmp(pp, TJS_W("file://"), 7 * sizeof(tjs_char))) {
			const tjs_char *r = pp + 7;
			const tjs_char *q = r;
			while (*q && ((*q >= TJS_W('a') && *q <= TJS_W('z')) || (*q >= TJS_W('A') && *q <= TJS_W('Z')))) q++;
			if (*q == TJS_W(':'))
			{
				name = ttstr(r);
				return;
...
{
#ifdef __SWITCH__
	// KRKR-ns patch: in game mode, bare (relative) names resolve under the
	// game directory. The emulator's fs layer ignores chdir(), so relying on
	// the process cwd breaks relative lookups there; this restores the KRKR
	// data-path semantics in both environments.
	extern bool krkrsdl2_game_mode;
	extern ttstr krkrsdl2_game_dir;
	if (krkrsdl2_game_mode && !name.IsEmpty())
	{
		const tjs_char *s = name.c_str();
		bool bare = true;
		if (s[0] == TJS_W('?') && s[1] == TJS_W('/')) bare = false; // "?<dev>/..." alias
		if (ns_gp_is_bare(s) == false) bare = false;               // has scheme/drive
		if (bare)
		{
			name = krkrsdl2_game_dir + name;
		}
	}
#endif
	ttstr tmp = name;
...
#ifdef __SWITCH__
	// KRKR-ns patch: the storage normalize passes sometimes yield a leading
	// "/" before an fsdev absolute device path ("/sdmc:/...") which fsdev
	// cannot open; strip it ("/sdmc:x" → "sdmc:x").
	if (!tmp.IsEmpty() && tmp.c_str()[0] == TJS_W('/'))
	{
		const tjs_char *s = tmp.c_str() + 1;
		const tjs_char *q = s;
		while (*q && ((*q >= TJS_W('a') && *q <= TJS_W('z')) || (*q >= TJS_W('A') && *q <= TJS_W('Z')))) q++;
		if (*q == TJS_W(':'))
		{
...
#if defined(__SWITCH__) || defined(__vita__)
	// HACK for Switch and Vita: a device path such as sdmc:/... contains a
	// colon, but it is still native filesystem syntax and needs file://?/.
	// Registered storage-media URLs (psb://, http://, ...) already use the TVP
	// naming grammar and must pass through untouched.
	const bool has_uri_scheme =
		TJS_strstr(name.c_str(), TJS_W("://")) != nullptr;
	// IncludeTrailingSlash can turn romfs:/ into romfs://; that is still a
	// libnx device path rather than a TVP media scheme.
	const bool is_switch_device =
		name.StartsWith(TJS_W("romfs:")) ||
		name.StartsWith(TJS_W("sdmc:")) ||
		name.StartsWith(TJS_W("save:")) ||
		name.StartsWith(TJS_W("host:")) ||
		name.StartsWith(TJS_W("usb:"));
	if ((!has_uri_scheme || is_switch_device) &&
		!ns_gp_is_bare(name.c_str()))
	{
...
ttstr TVPGetAppPath()
{
	// KRKR-ns patch: no static caching — ExePath() is mount-dependent in
	// game mode (romfs before switch_game_mount, sdmc game dir after), and
	// System.exepath (used by KAG to locate plugin/) must see the current
	// value. Cache would pin the pre-mount romfs path forever.
	return TVPExtractStoragePath(TVPNormalizeStorageName(ExePath()));
}
//---------------------------------------------------------------------------
...
tTVPArchive * TVPOpenArchive(const ttstr & name)
{
	...
	// Archive type is determined from its contents, not its extension.  Some
	// Kirikiroid-targeted games deliberately name a standard 7-Zip resource
	// package "patch.xp3" and mount it through the normal auto-path API.
	tTJSBinaryStream *stream = TVPCreateStream(name);
	archive = TVPOpen7ZArchive(name, stream);
	if(archive) return archive; // the 7z archive owns stream
	delete stream;
...
static bool _TVPCreateFolders(const ttstr &folder)
{
	...
	if( TVPUtf16ToUtf8( filename, folder.AsStdString() ) ) {
		KRKRNS_LOG("[fs] media mkdir: folderstr=[%s] len=%d", filename.c_str(), (int)filename.size());
		if (filename.empty() || filename.c_str() == nullptr) {
			KRKRNS_LOG("[fs] media mkdir: path empty or null, skipping");
			return false;
		}
#if defined(__vita__)
...
#endif
		KRKRNS_LOG("[fs] media mkdir res=%d", res);
	} else {
		KRKRNS_LOG("[fs] media mkdir: UTF8 conversion failed, folder empty?");
	}
	return 0 == res;
#endif
}
