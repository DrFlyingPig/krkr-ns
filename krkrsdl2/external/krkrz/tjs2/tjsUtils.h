...
{
#ifdef KRKRZ_USE_SDL_THREADS
	// SDL mutexes are not recursive, but engine code (archive open inside
	// TVPCreateStream's storage critical section, etc.) legitimately re-enters
	// critical sections on the same thread.  Windows CRITICAL_SECTION, which
	// the Win32 build uses, is recursive -- so emulate that here: owner thread
	// + nest count.  Only the owner touches Owner/Count while holding the
	// mutex, so the pre-lock check on other threads is a benign read.
	SDL_mutex *Mutex;
...
public:
	...
	void Leave()
	{
		SDL_threadID self = SDL_ThreadID();
		if (Owner != self)
			return; // broken usage; do not unlock a foreign mutex
		if (--NestCount == 0)
		{
			Owner = 0;
