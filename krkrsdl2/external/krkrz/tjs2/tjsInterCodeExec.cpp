...
{
#ifdef __SWITCH__
	// KRKR-ns diagnostic: trace KAG boot calls (eval/exec/execStorage)
	// so a VM that stops between exec and execStorage is visible.
	{
		tTJSVariant *nm = TJS_GET_VM_REG_ADDR(DataArea, code[3]);
		if (nm && nm->Type() == tvtString)
		{
			const tjs_char *s = nm->GetString();
			if (s && (s[0] == 'e') && s[1] == 'x' && s[2] == 'e' && s[3] == 'c')
			{
				KRKRNS_LOG("[vmcalld] %ls", s);
			}
		}
	}
#endif
