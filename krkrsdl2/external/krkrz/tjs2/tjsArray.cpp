...
//----------------------------------------------------------------------
TJS_BEGIN_NATIVE_METHOD_DECL(/* func. name */save2)
{
	// Compatibility with saveStruct.dll's Array.save2.  Several commercial
	// titles use this instead of Array.save when writing configuration files.
	// The original plugin writes raw tjs_char strings and lets the third
	// argument select LF (true) or CRLF (false); its documented UTF argument
	...
	ttstr name(*param[0]);
	const bool only_lf = numparams >= 3 && param[2]->Type() != tvtVoid &&
		(tjs_int)*param[2] != 0;
	const tjs_char * newline = only_lf ? TJS_W("\n") : TJS_W("\r\n");
	const tjs_uint newline_bytes =
		(tjs_uint)(TJS_strlen(newline) * sizeof(tjs_char));

	iTJSBinaryStream * stream = TJSCreateBinaryStreamForWrite(name, ttstr());
	try
	{
		for(tTJSArrayNI::tArrayItemIterator i = ni->Items.begin();
			i != ni->Items.end(); ++i)
		{
			const tjs_char * text = i->GetString();
			if(text)
			{
				stream->Write(text,
					(tjs_uint)(TJS_strlen(text) * sizeof(tjs_char)));
			}
			stream->Write(newline, newline_bytes);
		}
	}
	catch(...)
	{
...
TJS_END_NATIVE_METHOD_DECL(/* func.name */save2)
//----------------------------------------------------------------------
