...
#include <cstdarg>
static void KrkrBltTrace(const char* fmt, ...)
{
	static int lines = 0;
	if (lines >= 400) return;
	lines++;
...
#ifdef __SWITCH__
        if (w * h >= 640 * 480)
            KrkrBltTrace("CopyRect dest=(%d,%d,%d,%d) src=(%d,%d,%d,%d) self=%dx%d ref=%dx%d",
...
#ifdef __SWITCH__
        if (w * h >= 640 * 480)
            KrkrBltTrace("Blt dest=(%d,%d)+%dx%d src=(%d,%d,%d,%d) self=%dx%d ref=%dx%d method=%d",
                rect.left, rect.top, w, h,
                refrect.left, refrect.top, refrect.right, refrect.bottom,
                GetWidth(), GetHeight(), ref->GetWidth(), ref->GetHeight(), (int)method);
#endif
			...
			bool hda, tTVPBBStretchType mode, tjs_real typeopt )
{
	...
	{
#ifdef __SWITCH__
		KrkrBltTrace("StretchBlt>resample dest=(%d,%d,%d,%d) src=(%d,%d,%d,%d) self=%dx%d ref=%dx%d type=%d method=%d opa=%d",
			destrect.left, destrect.top, destrect.right, destrect.bottom,
			refrect.left, refrect.top, refrect.right, refrect.bottom,
			GetWidth(), GetHeight(), ref->GetWidth(), ref->GetHeight(),
			(int)type, (int)method, opa);
#endif
...
#ifdef __SWITCH__
        KrkrBltTrace("AffineBlt dest=(%d,%d,%d,%d) srcrect=(%d,%d,%d,%d) p0=(%.2f,%.2f) p1=(%.2f,%.2f) p2=(%.2f,%.2f) sxstep=%.3f systep=%.3f lr=(%.2f,%.2f) scan=(%.2f,%.2f) taskNum=%d self=%dx%d ref=%dx%d method=%d",
            destrect.left, destrect.top, destrect.right, destrect.bottom,
            srcrect.left, srcrect.top, srcrect.right, srcrect.bottom,
            points_x[0] / 65536.0, points_y[0] / 65536.0,
            points_x[1] / 65536.0, points_y[1] / 65536.0,
            points_x[2] / 65536.0, points_y[2] / 65536.0,
            sxstep / 65536.0, systep / 65536.0,
            leftlimit / 65536.0, rightlimit / 65536.0,
            scanlinestart / 65536.0, scanlineend / 65536.0,
            GetAdaptiveThreadNum(w * h, sBmFactor[method] * 13 / 59),
            GetWidth(), GetHeight(), ref->GetWidth(), ref->GetHeight(),
            (int)method);
#endif
