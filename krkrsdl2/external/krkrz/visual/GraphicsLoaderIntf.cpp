...
// bytes are PNG or WebP. Route built-in formats by signature so a virtual
// filename cannot select the wrong decoder.
static void TVPLoadGraphicBySignature(void *formatdata, void *callbackdata,
	tTVPGraphicSizeCallback sizecallback,
	tTVPGraphicScanLineCallback scanlinecallback,
	tTVPMetaInfoPushCallback metainfopushcallback,
	tTJSBinaryStream *source, tjs_int32 keyidx, tTVPGraphicLoadMode mode)
{
	tjs_uint8 header[16] = {};
	const tjs_uint64 original = source->GetPosition();
	const tjs_uint read = source->Read(header, sizeof(header));
	source->SetPosition(original);
	if (read >= 2 && !std::memcmp(header, "BM", 2))
		return TVPLoadBMP(formatdata, callbackdata, sizecallback, scanlinecallback,
			metainfopushcallback, source, keyidx, mode);
	if (read >= 8 && !std::memcmp(header, "\x89PNG\r\n\x1a\n", 8))
		return TVPLoadPNG(formatdata, callbackdata, sizecallback, scanlinecallback,
			metainfopushcallback, source, keyidx, mode);
	if (read >= 3 && !std::memcmp(header, "TLG", 3))
		return TVPLoadTLG(formatdata, callbackdata, sizecallback, scanlinecallback,
			metainfopushcallback, source, keyidx, mode);
	if (read >= 3 && header[0] == 0xff && header[1] == 0xd8 && header[2] == 0xff)
		return TVPLoadJPEG(formatdata, callbackdata, sizecallback, scanlinecallback,
			metainfopushcallback, source, keyidx, mode);
#ifdef TVP_IMAGE_ENABLE_WEBP
	if (read >= 12 && !std::memcmp(header, "RIFF", 4) && !std::memcmp(header + 8, "WEBP", 4))
		return TVPLoadWEBP(formatdata, callbackdata, sizecallback, scanlinecallback,
			metainfopushcallback, source, keyidx, mode);
#endif
#ifdef _WIN32
	if (read >= 4 && !std::memcmp(header, "\x49\x49\xbc\x01", 4))
		return TVPLoadJXR(formatdata, callbackdata, sizecallback, scanlinecallback,
			metainfopushcallback, source, keyidx, mode);
#endif
	TVPThrowExceptionMessage(TVPImageLoadError, TJS_W("Invalid image signature"));
}

static void TVPLoadHeaderBySignature(void *formatdata, tTJSBinaryStream *source,
	iTJSDispatch2 **dictionary)
{
	tjs_uint8 header[16] = {};
	const tjs_uint64 original = source->GetPosition();
	const tjs_uint read = source->Read(header, sizeof(header));
	source->SetPosition(original);
	if (read >= 2 && !std::memcmp(header, "BM", 2)) return TVPLoadHeaderBMP(formatdata, source, dictionary);
	if (read >= 8 && !std::memcmp(header, "\x89PNG\r\n\x1a\n", 8)) return TVPLoadHeaderPNG(formatdata, source, dictionary);
	if (read >= 3 && !std::memcmp(header, "TLG", 3)) return TVPLoadHeaderTLG(formatdata, source, dictionary);
	if (read >= 3 && header[0] == 0xff && header[1] == 0xd8 && header[2] == 0xff) return TVPLoadHeaderJPG(formatdata, source, dictionary);
#ifdef TVP_IMAGE_ENABLE_WEBP
	if (read >= 12 && !std::memcmp(header, "RIFF", 4) && !std::memcmp(header + 8, "WEBP", 4)) return TVPLoadHeaderWEBP(formatdata, source, dictionary);
#endif
#ifdef _WIN32
	if (read >= 4 && !std::memcmp(header, "\x49\x49\xbc\x01", 4)) return TVPLoadHeaderJXR(formatdata, source, dictionary);
#endif
	TVPThrowExceptionMessage(TVPImageLoadError, TJS_W("Invalid image signature"));
}
...
public:
	...
	tTVPGraphicType()
	{
		...
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".bmp"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsBMP, TVPAcceptSaveAsBMP, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".dib"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsBMP, TVPAcceptSaveAsBMP, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".jpeg"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsJPG, TVPAcceptSaveAsJPG, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".jpg"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsJPG, TVPAcceptSaveAsJPG, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".jif"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsJPG, TVPAcceptSaveAsJPG, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".png"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsPNG, TVPAcceptSaveAsPNG, NULL));
#ifdef TVP_IMAGE_ENABLE_WEBP
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".webp"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, NULL, NULL, NULL));
#endif
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".tlg"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsTLG, TVPAcceptSaveAsTLG, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".tlg5"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsTLG, TVPAcceptSaveAsTLG, NULL));
		Handlers.push_back(tTVPGraphicHandlerType(
			TJS_W(".tlg6"), TVPLoadGraphicBySignature, TVPLoadHeaderBySignature, TVPSaveAsTLG, TVPAcceptSaveAsTLG, NULL));
#ifdef _WIN32
		Handlers.push_back(tTVPGraphicHandlerType(
