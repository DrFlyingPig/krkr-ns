...
// windowEx.dll exposes this as a writable Window property.  Several KAGEX
// games assign the generated system-menu model during Window construction even
// when there is no native desktop menu.  Keep the value on the TJS instance so
// scripts observe the original contract while the Switch backend remains a
// single SDL window.
TJS_BEGIN_NATIVE_PROP_DECL( exSystemMenu ) {
	TJS_BEGIN_NATIVE_PROP_GETTER
	{
		tTJSVariant value;
		if(TJS_FAILED(objthis->PropGet(TJS_MEMBERMUSTEXIST,
			TJS_W("_krkrnsExSystemMenu"), NULL, &value, objthis)))
			result->Clear();
		else
			*result = value;
		return TJS_S_OK;
	...
	TJS_BEGIN_NATIVE_PROP_SETTER
	{
		return objthis->PropSet(TJS_MEMBERENSURE,
			TJS_W("_krkrnsExSystemMenu"), NULL, param, objthis);
	}
	TJS_END_NATIVE_PROP_SETTER
}
TJS_END_NATIVE_PROP_DECL( exSystemMenu )
#endif
//----------------------------------------------------------------------
