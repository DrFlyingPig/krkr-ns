	...
	try
	{
		...
		while(true)
		{
				...
				TVP_XP3_INDEX_ENCODE_ZLIB)
			{
				...
				try
				{
					KRKRNS_STAGE("xp3: inflate index");
			...
			KRKRNS_STAGE("xp3: parse entries");
...
//---------------------------------------------------------------------------
// KRKR-ns Phase 1: segment cache was 1MB total — on a 3GB-applet Switch this
// evicted hot segments (backgrounds/sprites) constantly during scene changes.
// 8MB per segment, 16MB total keeps the current scene's compressed data warm.
#define TVP_SEGCACHE_ONE_LIMIT (8*1024*1024)    // max size limit for each segment
#define TVP_SEGCACHE_TOTAL_LIMIT (16*1024*1024) // total segment cache size
