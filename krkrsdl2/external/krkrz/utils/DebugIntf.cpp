...
#ifdef __SWITCH__
	// KRKR-ns patch: mirror engine log lines to the SD-card debug log
	{
		krkrsdl2_logf_impl("%s", line.AsNarrowStdString().c_str());
	}
#endif
}
//---------------------------------------------------------------------------
...
void TVPSetLogLocation(const ttstr &loc)
{
	...
	KRKRNS_LOG("[log] TVPSetLogLocation: loc=[%s]", loc.AsStdString().c_str());
	ttstr native = TVPGetLocallyAccessibleName(TVPLogLocation);
	KRKRNS_LOG("[log] TVPSetLogLocation: native=[%s] empty=%d", native.AsStdString().c_str(), (int)native.IsEmpty());
	if(native.IsEmpty())
	{
