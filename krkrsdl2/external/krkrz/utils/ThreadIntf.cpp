...
void DrawThreadPool::PoolThread( tjs_int taskNum ) {
	...
	static bool firstPoolSpawn = false;
	if (!firstPoolSpawn)
	{
		firstPoolSpawn = true;
		KRKRNS_LOG("[pool] spawned workers=%d taskNum=%d threadNum=%d",
			(int)workers.size(), taskNum, TVPGetThreadNum());
	}
	// KRKR-ns Phase 2: the upstream shrink loop below destroyed every worker
	// whenever a 1-task batch arrived (the common case — most layer ops are
	// small, and PoolThread runs on EVERY TVPBeginThreadTask), so real
	// parallel batches re-created their threads from scratch each time:
	// constant create/teardown churn on the main thread (device log showed
	// ~270k BeginTask calls with avgNum=1.0). Keep workers alive for the
	// session instead; 3 idle threads are cheap on the Switch.
	}
//---------------------------------------------------------------------------
static DrawThreadPool TVPTheadPool;
//---------------------------------------------------------------------------
// KRKR-ns Phase 2: lifetime pool counters for the [prof] emitter in
// SDLApplication.cpp (per-window task counts reveal E-mote scene layer-op
// load: hundreds of small ops per frame stay single-threaded on purpose).
static unsigned gPoolBeginsTotal = 0;
static unsigned gPoolBigBegins = 0;
extern "C" unsigned krkrsdl2_pool_begins() { return gPoolBeginsTotal; }
extern "C" unsigned krkrsdl2_pool_bigbegins() { return gPoolBigBegins; }
//---------------------------------------------------------------------------
void TVPBeginThreadTask( tjs_int taskNum ) {
	// KRKR-ns Phase 1b diag: who drives the pool and how wide are the tasks
	static Uint32 poolBegins = 0;
	static Uint64 poolTaskSum = 0;
	static tjs_int poolTaskMax = 0;
	poolBegins++; poolTaskSum += (Uint64)taskNum;
