# KRKR-ns 开发记录（2026-09-03 起）

本文档保留各阶段的实现与排查记录；历史章节中的“当前状态”和“待办”描述当时情况，最新进展见本节和 README。配套文档:
- `PORTING_PLAN.md` — 已批准的移植计划(阶段/里程碑/风险)
- `PATCHES.md` — 全部代码层补丁的技术清单(本文踩坑处均有对应补丁条目)
- [MENU_PERFORMANCE.md](MENU_PERFORMANCE.md) — 菜单公共路径优化及性能对比（P62）
- [FREEZE_DIAGNOSIS.md](FREEZE_DIAGNOSIS.md) — 长剧情卡死、字体生命周期与选项居中（P63）

## 2026-09-12：菜单性能与长剧情稳定性（P62 / P63）

- 菜单首次点击涉及的字体度量改为按需分页，结构化存档合并细碎写入，PSB 共享只读资源并预留字典容量，固实 7z 复用最近解压块；补齐图像缓存计费与预算限制。
- 真机日志中出现大位图分配失败，模拟器进一步复现“通用堆约 2.38 GiB 空闲，却不能分配约 17 MiB 连续位图”。Switch 大图改用独立内存区复用，空闲相邻页可合并；增加位图实际占用、堆空闲量和分配重试日志。
- 排版回归发现第二处崩溃：临时文字对象析构时关闭全局 FreeType 库，破坏其他仍存活的字体。全局库改在引擎最终清理阶段释放，实例仅释放自己的 face。
- 补齐 TextRender `valign=0` 的居中分支，使用当前字体 ascent；用户已通过选项画面截图确认。
- 验证：5 项本地测试和 22,813 项 ARM 引擎校验通过，包含四线程共 6,000 轮位图分配释放、300 轮大图写时复制、16 轮文字对象创建销毁。与 P62 相比，31 个字号的文字宽度及六种存档文件字节一致。
- 位图内存区版本约 18 分钟模拟器剧情测试未再出现位图分配失败，包含快进、暂停和选项等待；最终字体补丁另经完整引擎回归。真机菜单延迟和最终版长期稳定性待复验，不能将模拟器结果视为真机验收。
- 正式产物保持 `build-switch/krkrsdl2.nro`，构建信息与哈希见排查记录。模拟器 91 个存档文件已恢复并逐文件校验；MTP 仅用于读取真机日志。
