...
> 📋 **开发全过程复盘(需求/流程/踩坑)、诊断与性能记录保存在本地开发文档（未随仓库发布）**——本文件只列代码补丁的技术细节。

相对上游 krkrsdl2 (pinned `bf207f2`) 的全部本地改动。同步上游时逐个重新应用。

> 📐 逐文件的差异清单（自动生成，含内嵌 krkrz 层 vs pinned `b11c43a`）见 [UPSTREAM_DELTA.md](UPSTREAM_DELTA.md)，可在同步上游前用它核对范围。
